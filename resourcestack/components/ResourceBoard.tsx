...
export function ResourceBoard() {
  ...
  async function load() {
    ...
    setError(null);
    try {
      const data = await apiFetch<{ resources: Resource[] }>(`/api/resources?search=${encodeURIComponent(search)}&tag=${encodeURIComponent(tag)}`);
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
      <section className="rounded-3xl bg-zinc-900/60 p-5 shadow-soft ring-1 ring-white/10">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-semibold">Add a resource</h2>
          <div className="text-xs text-zinc-300">Links, docs, tools</div>
        </div>
        ...
        <form onSubmit={createResource} className="mt-4 space-y-3">
          ...
          <div>
            <label className="text-xs text-zinc-300">Notes (optional)</label>
            <textarea
              value={form.notes}
          ...
          <button
            type="submit"
            className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-white px-3 py-2 text-sm font-semibold text-zinc-950 hover:bg-zinc-100"
          >
          ...
          <div className="rounded-2xl bg-zinc-950/40 p-3 text-xs text-zinc-300 ring-1 ring-white/10">
            Tip: Use the star to pin favorites at the top.
          </div>
        </form>
      </section>

      <section className="rounded-3xl bg-zinc-900/40 p-5 shadow-soft ring-1 ring-white/10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-base font-semibold">Your library</h2>
            <div className="text-xs text-zinc-300">{filtered.length} items</div>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-2xl bg-zinc-950/60 py-2 pl-9 pr-3 text-sm ring-1 ring-white/10 outline-none focus:ring-2 focus:ring-white/20 sm:w-64"
              />
            </div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full appearance-none rounded-2xl bg-zinc-950/60 py-2 pl-9 pr-8 text-sm ring-1 ring-white/10 outline-none focus:ring-2 focus:ring-white/20 sm:w-48"
              >
        ...
        <div className="mt-5 grid gap-3">
          {filtered.map((r) => (
            <div key={r.id} className="rounded-3xl bg-zinc-950/40 p-4 ring-1 ring-white/10 hover:bg-zinc-950/50">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <Link href={`/resources/${r.id}`} className="no-underline">
                    <div className="truncate text-sm font-semibold hover:underline">{r.title}</div>
                  </Link>
                  <a
                    href={r.urlOriginal}
                    target="_blank"
                    rel="noreferrer"
                    className="mt-1 inline-flex max-w-full items-center gap-2 truncate text-xs text-zinc-300 hover:text-zinc-100"
                  >
                    <ExternalLink className="h-3.5 w-3.5" />
                    <span className="truncate">{r.urlOriginal}</span>
                  </a>
                  {r.notes ? <div className="mt-2 text-sm text-zinc-200/90">{r.notes}</div> : null}
                  <div className="mt-3 flex flex-wrap gap-2">
                    {r.tags.map((t) => (
                      <span key={t} className="rounded-full bg-white/10 px-2 py-1 text-xs text-zinc-200 ring-1 ring-white/10">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <button
                  ...
                  className="grid h-10 w-10 place-items-center rounded-2xl bg-white/5 ring-1 ring-white/10 hover:bg-white/10"
                  title={r.isFavorite ? "Unfavorite" : "Favorite"}
                >
                  <Star className={`h-4 w-4 ${r.isFavorite ? "fill-yellow-300 text-yellow-300" : "text-zinc-200"}`} />
                </button>
              </div>
            </div>
          ))}
          ...
          {filtered.length === 0 ? (
            <div className="rounded-3xl bg-zinc-950/30 p-10 text-center text-sm text-zinc-300 ring-1 ring-white/10">
